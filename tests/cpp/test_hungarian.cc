...
double total_cost(const Eigen::MatrixXd &cost, const std::vector<std::pair<int, int>> &assignment) {
  double total = 0.0;
...
TEST_CASE("Hungarian: all cells infeasible still returns valid pairs", "[hungarian]") {
...
TEST_CASE(
    "Hungarian: rectangular with one fully-infeasible row demonstrates "
    "caller filter pattern",
    "[hungarian]") {
  Eigen::MatrixXd cost(2, 3);
  cost << immtrack::detail::kInfeasible, immtrack::detail::kInfeasible,
      immtrack::detail::kInfeasible, 1.0, 2.0, 3.0;
