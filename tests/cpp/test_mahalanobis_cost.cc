...
TEST_CASE("MahalanobisCost: positive distance for offset observation", "[cost][mahalanobis]") {
...
TEST_CASE("MahalanobisCost: yaw wraps correctly across pi boundary", "[cost][mahalanobis]") {
...
TEST_CASE("MahalanobisCost: gate threshold matches chi^2(4, 0.99)", "[cost][mahalanobis]") {
  REQUIRE_THAT(Cost::gate_threshold(), WithinAbs(13.28, 1e-6));
}

TEST_CASE("MahalanobisCost: distance grows with observation offset", "[cost][mahalanobis]") {
