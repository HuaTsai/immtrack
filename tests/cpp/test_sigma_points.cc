...
TEMPLATE_TEST_CASE_SIG("Sigma points reconstruct mean and cov", "[sigma]", ((int N), N), 3, 5, 7,
                       10) {
...
TEST_CASE("generate_sigma_points throws on negative-definite cov", "[sigma]") {
  ...
  REQUIRE_THROWS_AS(generate_sigma_points<N>(mu, sigma, weights.lambda),
...
TEST_CASE("UnscentedWeights row sums equal 1 for mean weights", "[sigma]") {
  ...
  REQUIRE_THAT(weights.mean_weights.sum(), Catch::Matchers::WithinAbs(1.0, 1e-9));
}
