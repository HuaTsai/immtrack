...
void bind_filter(py::module_ &m, const char *name) {
  py::class_<Filter>(m, name)
      .def(py::init<>())
      .def(py::init<double, double, double>(), py::arg("alpha") = 1e-3, py::arg("beta") = 2.0,
           py::arg("kappa") = 0.0)
      ...
      .def_property_readonly_static("N", [](py::object) { return Filter::N; })
      .def_property_readonly_static("M", [](py::object) { return Filter::M; });
}
...
PYBIND11_MODULE(_core, m) {
  ...
  py::register_exception<immtrack::CovarianceNotPsd>(m, "CovarianceNotPsd", PyExc_RuntimeError);
  py::register_exception<immtrack::InvalidArgument>(m, "InvalidArgument", PyExc_ValueError);
  py::register_exception<immtrack::NumericalError>(m, "NumericalError", PyExc_RuntimeError);

  using UkfPosVxyzYawCV = immtrack::UKF<immtrack::PosVxyzYawCV, immtrack::PosYawObs>;
  bind_filter<UkfPosVxyzYawCV>(m, "UkfPosVxyzYawCV");

  py::class_<immtrack::BoundingBox>(m, "BoundingBox")
      .def(py::init<>())
      .def(py::init([](double x, double y, double z, double l, double w, double h, double rot,
                       std::string class_name, double score, int track_id) {
           ...
           py::arg("x") = 0.0, py::arg("y") = 0.0, py::arg("z") = 0.0, py::arg("l") = 0.0,
           py::arg("w") = 0.0, py::arg("h") = 0.0, py::arg("rot") = 0.0,
           py::arg("class_name") = std::string(), py::arg("score") = 0.0, py::arg("track_id") = -1)
  ...
  using Tracker = immtrack::BBoxTracker<UkfPosVxyzYawCV, immtrack::MahalanobisCost>;

  py::class_<Tracker>(m, "BBoxTracker")
      .def(py::init([](int n_init, int max_age, int min_hits, double size_ema_alpha) {
             Tracker::Config c;
             c.n_init = n_init;
             c.max_age = max_age;
             c.min_hits = min_hits;
           ...
           py::arg("n_init") = 3, py::arg("max_age") = 5, py::arg("min_hits") = 3,
           py::arg("size_ema_alpha") = 0.7)
      .def("update",
           py::overload_cast<const std::vector<immtrack::BoundingBox> &>(&Tracker::update),
           py::arg("detections"))
      .def("update",
           py::overload_cast<const std::vector<immtrack::BoundingBox> &, double>(&Tracker::update),
  ...
  py::enum_<immtrack::metrics::MatchMetric>(metrics, "MatchMetric")
      .value("CenterDistance", immtrack::metrics::MatchMetric::CenterDistance)
      .value("Iou3d", immtrack::metrics::MatchMetric::Iou3d);

  py::class_<immtrack::metrics::AmotaConfig>(metrics, "AmotaConfig")
      .def(py::init<>())
      .def_readwrite("recall_values", &immtrack::metrics::AmotaConfig::recall_values)
      .def_readwrite("metric", &immtrack::metrics::AmotaConfig::metric)
      .def_readwrite("match_threshold", &immtrack::metrics::AmotaConfig::match_threshold);

  py::class_<immtrack::metrics::AmotaResult>(metrics, "AmotaResult")
      .def_readonly("overall", &immtrack::metrics::AmotaResult::overall)
      .def_readonly("per_class", &immtrack::metrics::AmotaResult::per_class);

  metrics.def("amota", &immtrack::metrics::amota, py::arg("gt"), py::arg("pred"),
