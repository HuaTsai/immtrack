...
inline double pair_cost(const BoundingBox& a,
                        const BoundingBox& b,
                        MatchMetric metric) {
        ...
        for (double target_recall : cfg.recall_values) {
            ...
            for (double cutoff : scores) {
                const auto cr = detail_amota::count_for_cutoff(
                    gt_frames, pred_frames, cls, cutoff, cfg.metric,
                    cfg.match_threshold);
            ...
            if (found) {
                const auto cr = detail_amota::count_for_cutoff(
                    gt_frames, pred_frames, cls, best_cutoff, cfg.metric,
                    cfg.match_threshold);
