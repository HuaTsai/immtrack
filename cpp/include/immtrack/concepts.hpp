...
template <class S>
concept StateSpace = requires(typename S::State x, typename S::Tangent dx, typename S::State a,
                              typename S::State b) {
  typename S::Cov;
...
template <class M>
concept MotionModel = requires(const typename M::StateSpace::State &x,
...
template <class M>
concept HasMotionJacobian = MotionModel<M> &&
...
template <class M>
concept LinearMotion = HasMotionJacobian<M> && requires(typename M::StateSpace::Scalar dt) {
...
template <class O>
concept ObservationModel = requires(const typename O::StateSpace::State &x) {
...
template <class O>
concept HasObsJacobian = ObservationModel<O> && requires(const typename O::StateSpace::State &x) {
  { O::H_jacobian(x) } -> std::same_as<ObsJacMat<O>>;
};

template <class O>
concept LinearObs = HasObsJacobian<O> && requires {
  { O::H_matrix() } -> std::same_as<ObsJacMat<O>>;
};

// 4. Filter — uniform external API for KF / EKF / UKF / IMM.
template <class F>
concept Filter = requires {
  typename F::StateSpace;
  typename F::MeasSpace;
}
&&StateSpace<typename F::StateSpace> &&StateSpace<typename F::MeasSpace> &&requires(
    F &f, double dt, const typename F::MeasSpace::State &z, const F &cf) {
